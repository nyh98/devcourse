...
router
  ...
  .get((req, res) => {
    ...
    if (!channelId) return res.status(400).send('데이터 없음');
...
router
  .route('/')
  .post(Validator.isStringFromBody('channelTitle, userId'), (req, res) => {
    const result = validationResult(req);

    if (!result.isEmpty()) return res.status(400).send('유효성 검사 실패');
